...
const SOPS = [
  'SOP-IMG-001: DICOM Annotation Standard',
  'SOP-CL-042: Cancer Tumor Analysis Protocol',
  'SOP-QA-017: Reader Calibration Checklist',
];
...
export default function Screen04() {
  ...
  const [teamName, setTeamName] = useState(workflowData.teamName || 'Cancer Tumor Analysis Team');
  const [caseAssignment] = useState('Cancer Tumor Analysis — Oncology Phase II TA Lung');
  const [datasetVersion, setDatasetVersion] = useState(workflowData.datasetVersion);
  const [members, setMembers] = useState<TeamMember[]>(workflowData.teamMembers);
  const [newName, setNewName] = useState('');
  const [newRole, setNewRole] = useState('');
  const [auditEnabled, setAuditEnabled] = useState(true);
  const [teamCreated, setTeamCreated] = useState(!!workflowData.teamName);
  const [hitlApproved, setHitlApproved] = useState(workflowData.hitlApproved);

  const canCreate = teamName.trim() && datasetVersion && members.length > 0;

  const addMember = () => {
    if (!newName.trim() || !newRole) return;
    setMembers(prev => [...prev, { id: Date.now().toString(), name: newName.trim(), role: newRole }]);
    setNewName('');
    setNewRole('');
  };
  ...
  const handleCreateTeam = () => {
    if (!canCreate) return;
    updateWorkflowData({ teamName, teamMembers: members, datasetVersion });
    setTeamCreated(true);
  };

  const handleApproveHITL = () => {
    updateWorkflowData({ hitlApproved: true });
    ...
    <div>
      ...
      {isDone && (
        <div className="screen-complete-banner">
          ✓ Reading team created. HITL workflows approved. Proceed to Reader Annotation.
        </div>
      )}
      ...
      <div className="two-col">
        <div className="card">
          ...
          <div className="form-group mb-12">
            ...
            <input className="form-input" value={teamName} onChange={e => setTeamName(e.target.value)} disabled={teamCreated} />
          </div>
          <div className="form-group mb-12">
            <label className="form-label">Case Assignment</label>
            <input className="form-input" value={caseAssignment} disabled style={{ color: 'var(--text-muted)' }} />
          </div>
          <div className="form-group">
            <label className="form-label">Dataset Version</label>
            <select className="form-select" value={datasetVersion} onChange={e => setDatasetVersion(e.target.value)} disabled={teamCreated}>
              ...
              <tbody>
                {members.map(m => (
                  <tr key={m.id}>
                    ...
                    {!teamCreated && (
                      <td>
                        <button className="btn btn-danger" style={{ padding: '4px 10px', fontSize: 11 }} onClick={() => removeMember(m.id)}>Remove</button>
                      </td>
                    )}
                  </tr>
                ))}
      ...
      <div className="card">
        ...
        <div className="two-col">
          <div>
            ...
            <ul className="check-list">
              {SOPS.map(sop => (
                <li key={sop} className="check-item">
        ...
        {teamCreated && (
          <div className="alert alert-ok mt-12">
            ✓ Team <strong>{teamName}</strong> created with {members.length} reader(s). Audit tracking: {auditEnabled ? 'enabled' : 'disabled'}.
          </div>
        )}
        {hitlApproved && (
          <div className="alert alert-ok mt-8">✓ HITL workflows approved. Readers notified and cases assigned.</div>
        )}
      </div>

      <div className="btn-group">
        <button className="btn btn-secondary" onClick={handleCreateTeam} disabled={!canCreate || teamCreated}>
          {teamCreated ? '✓ Team Created' : 'Create Team'}
        </button>
        <button className="btn btn-secondary" onClick={handleApproveHITL} disabled={!teamCreated || hitlApproved}>
          {hitlApproved ? '✓ HITL Approved' : 'Approve HITL Workflows'}
        </button>
        <button className="btn btn-primary" onClick={handleProceed} disabled={!hitlApproved || isDone}>
